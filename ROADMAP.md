...
Owner: "三国地图还是小看不清 / 买了地皮之后和没买要区分 / 设计一个方案显示谁买的"
Design approved (owner 2026-07-12):
1. **Fit-to-canvas spread** — after declutter, normalize flat-atlas positions to fill the whole
   board (relative geography kept; kills the dead half of the sanguo board).
2. **Bigger tiles** — flat-atlas tile + font scale up ~30-40% to match the new spacing.
3. **Ownership visualization** — bought tile: thick owner-color border + ~14px mini portrait FLAG
   on the corner (see WHO owns at a glance); unbought stays neutral-dark (contrast = the
   distinction); building pips unchanged; globe: owner-color ring on city points.
4. THEN Phase-7 bots (pre-game "add computer player") — still queued, also unblocks MCP acceptance.
