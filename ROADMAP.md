...
Owner: "游戏里地图占位置太少 / 界面要重新画 / 人脸小头像要在游戏里体现，否则骰子投完看不清谁动"
Scope decided: LAYOUT REBUILD, map-dominant + portrait tokens, keep the GB pixel style:
1. **Map-dominant layout** — board becomes the screen's primary element; side panels shrink/collapse
   (log/chat into drawers or tabs).
2. **Portrait tokens** — replace letter-initial tokens with character pixel portraits (portraits/
   already exist per mod) so you can SEE who moved; scale with the bigger board.
3. Keep: GB pixel aesthetic, all three renderers, the new animation/SFX layer.
