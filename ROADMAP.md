...
Owner: "还是很不清楚 请设置为全屏 只有这样才看的地图 / 地球类型也有这样的问题 /
chat log 等设置成鼠标悬浮后显示或者点击后显示 再点击一下收起"
Direction: the board fills the ENTIRE viewport edge-to-edge (flat atlas AND globe); ALL chrome
floats OVER the map (chips top overlay, action bar bottom overlay, drawer tabs as today) with
hover/click-toggle show-hide. This is the original "Option B full-bleed" layout the strips
approach didn't go far enough on.
