...
Owner: "界面实在太单调了 游戏界面要吸引人否则没人玩 全部重构比较好" — agreed scope: full VISUAL
rebuild on top of the existing architecture (renderers/anim/audio/popovers/tests stay).
Process: DESIGN FIRST — art-direction candidates + 2-3 complete in-game mockups for the owner to
choose BEFORE any code. Directions to explore: antique-map/ink for sanguo-style worlds, satellite
night-lights for terra-style, rich board-game table look; ties into the STANDING 2026-06-19 goal
(real city/map imagery as board background, NOT pixel art). create-mod already generates portraits
via gpt-image — per-world background art generation is the natural extension.
Then implementation waves: background art layer → tile/card redesign → HUD reskin → ambient motion.
